(function (Drupal, once) {
	Drupal.behaviors.asocoldermaExportActions = {
		attach(context) {
			const viewConfigs = [
				{
					selector: '.view-patrocinadores',
					tableKey: 'patrocinadores',
					exportUrl: '/admin/asocolderma/patrocinadores/export/excel'
				},
				{
					selector: '.view-asociados',
					tableKey: 'asociados',
					exportUrl: null
				},
				{
					selector: '.view-residentes',
					tableKey: 'residentes',
					exportUrl: null
				},
				{
					selector: '.view-proveedores',
					tableKey: 'proveedores',
					exportUrl: null
				},
				{
					selector: '.view-empleados',
					tableKey: 'empleados',
					exportUrl: null
				}
			];
			...
			viewConfigs.forEach(function (config) {
				once('asocolderma-export-actions-' + config.tableKey, config.selector, context).forEach(function (view) {
					...
					if (actionButton && actionSelect && !actionButton.dataset.asocoldermaBound) {
						...
						actionButton.addEventListener('click', function () {
							...
							if (action === 'excel') {
								...
								window.location.href = config.exportUrl + '?' + query.toString();
								return;
							}
